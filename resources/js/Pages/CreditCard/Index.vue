                    ...
                    <div class="relative overflow-x-auto shadow-md sm:rounded-lg">
                        <table class="w-full text-sm text-left rtl:text-right text-gray-500">
                            <thead class="text-xs text-jp-indigo uppercase bg-azureish-white border-b">
                                <tr>
                                    <th v-for="col in consts.credit_cards_cols" class="px-6 py-3">
                                        {{ col }}
                                    </th>
                            ...
                            <tbody>
                                <tr v-for="credit_card in credit_cards" :key="credit_card.id" class="bg-white border-b hover:bg-gray-50 dark:bg-gray-800 dark:hover:bg-independence">
                                    ...
                                    <td class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-gray-100">
                                        {{ credit_card.avail_credit_limit }}
                                    </td>
                                    <td class="px-6 py-4 space-x-2" v-if="form.role_id == 1">
                                        ...
                                        <Link :href="route('credit-cards.edit', credit_card.id)" class="font-medium text-blue-600 hover:underline pr-4">Edit</Link>
                                        <a href="#" class="font-medium text-red-600 hover:underline" @click.prevent="confirmDelete(credit_card, 'credit-cards')">Delete</a>
                                    </td>
                                </tr>
                            </tbody>
                        </table>
                        ...
                        <nav class="flex items-center flex-column flex-wrap md:flex-row justify-between py-2 px-4" aria-label="Table navigation">
                            <span class="text-sm font-normal text-gray-700 mb-4 md:mb-0 block w-full md:inline md:w-auto dark:text-gray-100">Showing <span class="font-semibold text-gray-700 dark:text-gray-100">1-{{ checkIfArrayExists(credit_cards) }}</span> of <span class="font-semibold text-gray-700 dark:text-gray-100">1</span></span>
